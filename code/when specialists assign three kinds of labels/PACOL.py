...
def selectInstance(label_ind, unlabel_ind, model):
    ...
    ret2 = int(dic[ret1])
    num=len(unlabel_ind)
    eva=np.zeros(num, dtype=np.float)
    ...
    for u in unlabel_ind.index:
        ...
        pvu2 = pro[v][2]
        eva0=abs(puv0-pvu0)
        eva1=1-abs(puv1-pvu1)
        eva2 = 1 - abs(puv2 - pvu2)
        eva[u]=eva0+eva1+eva2

    # select concept pairs whose evaluation function value is the maximum
    maxeva = 0
    for u in unlabel_ind.index:
        v = int(dic[u])
        if eva[u]> maxeva:
...
print("init model ")
model = OneVsRestClassifier( GradientBoostingClassifier(n_estimators=20,learning_rate=0.1,min_samples_split=300,min_samples_leaf=40,max_depth=13,max_features='sqrt'))
